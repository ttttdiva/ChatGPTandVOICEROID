...
class WebSearch:
    def __init__(self, google_api_key, cx, ai_name, model):
        # 設定ファイルを読み込み
    ...
    # メイン処理
    def bing_gpt(self, question, system_prompt):
...
"""
            gpt_chat = LLMManager(system_prompt, summary_prompt, ai_name=self.ai_name[0])
    ...
    def generate_search_word(self, question):
        # OpenAI=function callingの場合、既に検索ワード化している為
        if self.model.startswith('gpt'):
